...
def loadEncryptedImg(filename):
    ...
    fstream.close()
    return cipherimg
pub,pri=Paillier.generate_keys(128)
print(pub,pri)
with Image.open("00001.jpg") as im:
    print(im)
    cipherimg=ImgEncrypt(pub,im)
    print("bie")
    #cipherimg=homomorphicBrightness(pub,cipherimg,150)
    #print("bright")
    saveEncryptedImg(cipherimg,"TEST.jpg")
    print("saved")
    cipherimg=loadEncryptedImg("TEST.jpg")
    print("done")
    dec=ImgDecrypt(pub,pri,cipherimg)
    print(dec)
    dec.show()
    print("final")
